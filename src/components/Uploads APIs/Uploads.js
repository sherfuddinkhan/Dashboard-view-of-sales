...
const UPloads = () => {
  ...
  const createDestination = async () => {
    ...
    try {
      const res = await axios.post("http://localhost:5000/api/create-upload-destination", { accessToken });
  ...
  return (
    <div style={containerStyle}>
      ...
      <button onClick={createDestination} disabled={loading} style={styles.button}>
        {loading ? "Creating..." : "Create Upload Destination"}
      </button>
