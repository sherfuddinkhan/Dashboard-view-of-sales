...
const CreateFeedDocument = () => {
  ...
  const createFeedDocument = async () => {
    ...
    try {
      const response = await axios.post("http://localhost:5000/api/create-feed-document", {
        accessToken,
      });
  ...
  return (
    <div style={containerStyle}>
      <h2>Create Feed Document</h2>

      <label>Access Token</label>
      <textarea rows={5} value={accessToken} onChange={(e) => setAccessToken(e.target.value)} style={styles.textarea} />

      <button onClick={createFeedDocument} disabled={loading} style={styles.button}>
        {loading ? "Creating..." : "Create Feed Document"}
      </button>
      ...
      {result && (
        <>
          ...
          <button onClick={() => navigator.clipboard.writeText(result)} style={styles.copyButton}>
            Copy Result
          </button>
        </>
      )}
